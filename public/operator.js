...
function renderFeedResults(keyword, data, options = {}) {
  ...
  const xNote   = xCount  ? ` · ${xCount} �?�?`  : (!data.xConfigured      ? ' · <a href="#" onclick="showXSetup()" style="color:var(--accent);text-decoration:none">+ �?�?</a>' : '');
...
function applyFeedGridDiff(grid, allLeads) {
  ...
  for (const p of allLeads) {
    ...
    if (!card) {
      ...
      // Reorder in place if position changed — no DOM rebuild of the card itself
...
function allSubsSelected() {
  ...
  // → we don't need to send &subs=, let the server use its own default.
...
function renderSubsChips() {
  ...
  host.innerHTML = allSubsList().map(s => {
    ...
    return `<span class="feed-sub-chip${active ? ' active' : ''}" data-sub="${s}" title="${active ? 'Included in scan' : 'Excluded'}">
      r/${s}${extra ? `<span class="sub-close" data-remove="${s}" title="Remove custom sub">×</span>` : ''}
    </span>`;
  }).join('');
...
function buildPlatformBadge(post) {
  return post.platform === 'x'           ? `<span class="feed-platform feed-platform-x" title="X / Twitter">�?�?</span>`
  ...
  modal.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:28px 24px;width:340px;display:flex;flex-direction:column;gap:14px">
      <div style="font-size:1rem;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px">
        <span style="font-size:1.2rem">�?�?</span> Connect X / Twitter
      </div>
...
async function saveXToken() {
  ...
  toast('�?�? connected — fetch leads to see X results', 'ok');
}
